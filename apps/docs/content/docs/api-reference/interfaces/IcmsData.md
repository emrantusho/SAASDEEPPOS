...
[@saasdeep/fiscal](/docs/api-reference/index) / IcmsData
...
```ts
optional adRemICMSReten: number;
```
...
```ts
optional pICMSInterPart: number;
```
...
```ts
optional vICMSMonoReten: number;
```
...
```ts
optional vICMSSubstituto: number;
```
