...
Saasdeep Softwares is organized as a **Turborepo** monorepo with apps and shared packages.
...
│   └── www/                        # Landing page
├── packages/
│   └── fiscal/                     # @saasdeep/fiscal — standalone
│       └── src/
...
| `packages/fiscal` | `@saasdeep/fiscal` | Standalone Brazilian fiscal library (NF-e/NFC-e), zero database deps |
| `packages/ui` | `@saasdeep/ui` | Shared UI components (shadcn/Radix-based) |
| `packages/db` | `@saasdeep/db` | Drizzle ORM schema and database configuration |
| `packages/auth` | `@saasdeep/auth` | Better Auth factory and configuration |
| `packages/api` | `@saasdeep/api` | tRPC router infrastructure |
| `packages/env` | `@saasdeep/env` | Environment variable validation (t3-oss/env) |
